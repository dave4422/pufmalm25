...
class FewShotClassifierPUF(nn.Module):
    ...
    def functional_forward(self, x, weights):
        ...
        #print(x)
        #print(x.shape)
        x = F.linear(x,weights[f'layer1.0.weight'],weights[f'layer1.0.bias'])
        #print(x)
        #x = F.batch_norm(x, running_mean=None, running_var=None, weight=weights.get(f'layer1.1.weight'), bias=weights.get(f'layer1.1.bias'), training=True)
        x = F.relu(x)
        x = F.linear(x,weights[f'layer2.0.weight'],weights[f'layer2.0.bias'])
        ...
        x = F.linear(x, weights['logits.0.weight'], weights['logits.0.bias'])
        x = F.sigmoid(x)
